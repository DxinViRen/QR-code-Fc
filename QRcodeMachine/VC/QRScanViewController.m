...
//        [weakSelf.navigationController pushViewController:createQRCodeController animated:YES];
    ...
    _scanTool.scanFinishedBlock = ^(NSString *scanString) {
        ...
        if([scanString hasPrefix:@"http"]){
            ScanURlReopVC *scaUr= [[ScanURlReopVC alloc]init];
            scaUr.linestr = scanString;
            [weakSelf.navigationController pushViewController:scaUr animated:YES];
        }else{
            QrShowReVC *qrs = [[QrShowReVC alloc]init];
            qrs.data = scanString;
            [weakself.navigationController pushViewController:qrs animated:YES];
        }
        [[QRFacManager shareInstance]generScaNum];
